...
def exponentialAverage(means , variances):
  ...
  for layer in range(n_layers-1):
    averageMeans[layer] = (alpha * averageMeans[layer]) + ((1-alpha) * averageMeans[layer])
    averageVars[layer] = (alpha * averageVars[layer]) + ((1-alpha) * averageVars[layer])
...
def miniBatchGradientDescent(eta ,  W , b , gamma , beta):
  # load data
  ...
  for epoch in range(n_epochs): 
    ...
    for j in range( numBatches ):
      ...
      grad_b , grad_W  , grad_gamma , grad_beta= computeGradAnalytic(X_batch , Y_batch  ,W ,b , gamma , beta)
      for layer in range(n_layers):
